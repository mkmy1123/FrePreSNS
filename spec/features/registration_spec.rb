...
RSpec.feature "Registration funcrion", type: :feature do
  ...
  describe 'signup' do
    ...
    context "After resistration" do
      scenario "The user recieve welcome_mail" do
        visit root_path
        click_link "SIGN UP"
        fill_in "Email メールアドレス", with: "test@test.com"
        fill_in "Name 名前", with: "テストさん"
      ...
      scenario "The user's page exists" do
        sign_in user
        visit user_path(user)
        expect(current_url).to have_content(user.optional_id)
      end

      scenario "The user avator is default image" do
        visit user_path(user)
        expect(page).to have_css("img[src*='default']")
      end

      scenario "Other user is able to trust" do
        visit root_path
        click_link "SIGN UP"
        fill_in "Email メールアドレス", with: "test@test.com"
        fill_in "Name 名前", with: "テストさん"
  ...
  describe 'Logic delete' do
    ...
    context "By deleted user" do
      ...
      scenario "User's page exists, but consists of words: Deleted" do
        visit user_path(invalid_user)
        expect(page).to have_content "退会済み"
      end

      scenario "User's page don't has trust button" do
        sign_in user
        visit user_path(invalid_user)
